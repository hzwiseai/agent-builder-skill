...
员工训练和资产发布完成后，可以继续将已发布的 AI 员工绑定并发布到微信、
企业微信（企微）等已接入渠道，让真实客户通过对应渠道与员工对话。发布前需
在 WiseCopilot 中完成渠道授权、Agent 发布和渠道绑定；不同渠道的接入条件、
审核要求和消息能力以系统页面及[帮助中心](https://crm.wiseaio.com/help/manual)
的最新说明为准。Skill 负责训练、资产校验和发布准备，不会绕过渠道授权或替用户
发送未经确认的消息。
