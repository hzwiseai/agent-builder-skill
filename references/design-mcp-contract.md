...
| `design_get_knowledge_compile_input` | read | Read what a knowledge space contributes to position assets: reviewed (or active-release) knowledge whose category compiles into assets, grouped by compile target with asset references and applicable positions, required categories still empty, the AI members consuming the space with their position package, and the compile rules. Read it before proposing position draft changes through the harness tools; knowledge is the source, the position draft is the output. |
...
| `design_list_knowledge_templates` / `design_list_knowledge_template_items` / `design_get_knowledge_item_kind_templates` | read | Read the business templates (system and organization-owned) with categories, destinations and paired position/skill templates, one template's base content, and the authoring fields each knowledge kind requires. Build item fields from the kind template, never from memory. |
...
| `design_prepare_knowledge_template_item_save` / `design_commit_knowledge_template_item_save` | template content write | Add, revise or disable one base-content item of a business template, with its asset reference and positions. System templates are editable only by platform maintainers and organization templates by that organization's administrators; WorkTool enforces this. Spaces that imported earlier are not updated until they import again. |
