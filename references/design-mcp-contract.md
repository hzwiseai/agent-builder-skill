...
| `design_get_asset_edit_contract` | read | Return the server-derived editable surface, allowed operations, ownership and current baseline hash for one asset. Use it to render business-facing edit choices; it is not permission to write. |
| `design_get_asset_version_control` | read | Read draft-vs-published state and immutable published version history before proposing a restore or rollback. |
| `design_prepare_asset_version_restore` / `design_commit_asset_version_restore` | write | Review and restore an immutable published version into a draft with a concurrency check; restoration never publishes automatically. |
| `design_inspect_business_package` | read | Resolve an organization package's draft release preview and closure diagnostics. |
| `design_diagnose_business_package` | retired | Disabled for the skill-facing MCP. The skill diagnoses from bounded runtime/conversation evidence locally and does not call the system Copilot generator. |
...
| `design_list_knowledge_spaces` / `design_get_knowledge_space` / `design_get_knowledge_item` | read | List the organization's knowledge-center spaces, read one space's business-template coverage (categories, destinations, compile targets, missing required categories) with a page of active items, or read one item's content including its asset reference and applicable positions. |
...
| `design_prepare_knowledge_template_import` / `design_commit_knowledge_template_import` | knowledge draft write | Import the space's business-template base content as drafts; items the space already holds are skipped. The commit rejects a preview that changed. |
| `design_prepare_knowledge_space_template` / `design_commit_knowledge_space_template` | knowledge setting write | Switch the business template a space is organised by. Existing knowledge is kept; items whose category the new template lacks show as unmapped. |
| `design_prepare_knowledge_template_item_save` / `design_commit_knowledge_template_item_save` | template content write | Add, revise or disable one base-content item of a business template, with its asset reference and positions. System templates are editable only by platform maintainers and organization templates by that organization's administrators; the service enforces this. Spaces that imported earlier are not updated until they import again. |
...
| `design_prepare_local_knowledge_compile_receipt` / `design_commit_local_knowledge_compile_receipt` | audit receipt | Link a skill-local planner operation set to the matching active knowledge release and already-published business package. This records no model generation and writes no asset. |
...
| `design_prepare_business_publish` / `design_commit_business_publish` | publish | Capture a ready package preflight with publication impact, require the Eval gate for declared behavior changes, and publish only if the digest still matches after explicit confirmation. |
